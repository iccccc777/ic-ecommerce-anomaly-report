WITH first_active AS (
    SELECT
        user_id,
        MIN(date(timestamps, 'unixepoch', '+8 hours')) AS first_day
    FROM behavior_clean
    GROUP BY user_id
),
daily_active AS (
...
retained AS (
    SELECT
        f.first_day,
        a.day,
        CAST(julianday(a.day) - julianday(f.first_day) AS INTEGER) AS day_diff,
...
SELECT
    ...
    d.retained_users,
    ROUND(d.retained_users * 1.0 / c.users, 4) AS retention_rate
FROM cohort_size c
