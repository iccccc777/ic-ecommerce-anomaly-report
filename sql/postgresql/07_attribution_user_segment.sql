...
segment_counts AS (
    SELECT
        SUM(CASE WHEN active_1201 = 1 AND active_1202 = 1 THEN 1 ELSE 0 END)
            AS both_users,
        SUM(CASE WHEN active_1201 = 1 AND active_1202 = 0 THEN 1 ELSE 0 END)
            AS lost_users,
        SUM(CASE WHEN active_1201 = 0 AND active_1202 = 1 THEN 1 ELSE 0 END)
            AS gained_users
    FROM user_presence
)
...
FROM (
    SELECT
        ...
        0::BIGINT AS delta_contribution,
        ROUND(
            segment_counts.both_users::NUMERIC
